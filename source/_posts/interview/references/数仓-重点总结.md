...
1. **逻辑更连贯**：每段经历都按照 "负责什么→做了什么→解决了什么问题→带来了什么价值" 的结构展开，符合面试官的倾听习惯
2. **身份定位准确**：全程保持 "核心开发"、"主力开发"、"主导" 等符合 P7 级别的表述，没有夸大
...
项目经历方面呢，在去哪儿网期间，完整参与了服务平台的数据仓库的搭建，独立负责机票预定、支付、退改签核心域的ETL开发与数据建模，完成了日均 5 亿条数据的100多个离线任务的开发，搭建了覆盖6 0多张核心报表的业务流程报表体系，并且通过自动化数据质量校验将核心指标错误率从7%降至1%以下。
...
I have nearly 10 years of experience in big data development and enterprise-level data warehouse construction, focusing on offline and real-time data warehouse development and performance optimization for large-scale data scenarios.

During my tenure at Qunar, I fully participated in the construction of the data warehouse for the service platform. I independently took charge of ETL development and data modeling for the core domains of air ticket reservation, payment, refund and rescheduling. I developed more than 100 offline tasks processing 500 million daily data records, and built a business reporting system covering over 30 core business reports. Meanwhile, I implemented automated data quality verification, reducing the error rate of core indicators from 7% to below 1%.

At Meituan, I served as a core developer responsible for two key business domains: user and order. I independently completed the reconstruction of core fact tables and dimension tables, and unified the statistical caliber of more than 80 business indicators. Through data skew optimization, partition pruning and comprehensive performance tuning, I shortened the output time of core order reports from T+2 hours to T+45 minutes and reduced the task failure rate by 45%. In addition, I participated in the early-stage construction of the real-time data warehouse and developed multiple core real-time monitoring indicators using Flink.
...
Technically, I am proficient in core big data stacks including Hive, Spark, Flink and Kafka. My core strengths cover four key areas: first, enterprise-level data warehouse architecture design and dimensional modeling, with end-to-end experience in building offline and real-time data warehouses from scratch; second, performance tuning for PB-level large-scale data, with in-depth understanding of the underlying principles and optimization methods of Spark and Flink; third, data engineering implementation, skilled in CI/CD, containerization and automated operation and maintenance; fourth, data quality governance, with rich experience in building full-link data quality monitoring systems.
...
I have nearly 10 years of experience of big data development and enterprise-level data warehouse construction, focusing on offline and real-time data warehouse development and performance optimization for large-scale data scenarios.

Technically, I am proficient in core big data stacks including Hive, Spark, Flink and Kafka. My core strengths cover three key areas: first, enterprise-level data warehouse architecture design and dimensional modeling; second, performance tuning for PB-level large-scale data, with in-depth understanding of optimization methods of Spark and Flink; third, data quality governance, with rich experience in building full-link data quality monitoring systems.

With the above‑mentioned technical capabilities, I have delivered several end‑to‑end data projects.

During my tenure at Qunar, I fully participated in the construction of the data warehouse for the service platform. I independently took charge of ETL development and data modeling for the core domains of air ticket. I developed more than 100 offline tasks processing 500 million daily data records, and built a business reporting system covering over 60 core business reports.

At Meituan, I served as a core developer responsible for two key business domains: user and order. I independently completed the reconstruction of core fact tables and dimension tables, and unified the statistical caliber of more than 80 business indicators. In addition, I paticipated in the early-stage construction of the real-time data warehouse and developed multiple core real-time monitoring indicators using Flink.

During my work at Microsoft, I was in charge of MSN advertising data processing. I led the technical stack migration from Scope SQL to Spark Structured Streaming, migrating over 100 core offline and near-real-time tasks from Microsoft's proprietary Scope platform to the open-source Spark ecosystem. I also conducted full-link performance optimization. As a result, the average runtime of final tasks was reduced by 45%, and storage consumption decreased by 35%. Furthermore, I established an automated data quality monitoring system to realize automatic task deployment, testing and alerting, upgrading data anomaly detection from hour-level to minute-level.

This is all for my self-introduction. Thank you.

我拥有近十年的大数据开发及企业级数据仓库建设经验，专注于大规模数据场景下的离线和实时数据仓库的开发与性能优化。

在技术方面，我熟练掌握Hive、Spark、Flink和Kafka等核心大数据技术栈。我的核心优势涵盖三个关键领域：第一，企业级数据仓库架构设计与维度建模；第二，针对PB级大规模数据的性能调优，深入理解Spark和Flink的优化方法；第三，数据质量治理，具备构建全链路数据质量监控系统的丰富经验。

凭借上述技术能力，我已成功交付多个端到端的数据项目。

在加入Qunar期间，我全面参与了服务平台的数据仓库建设工作，独立负责航空机票核心业务域的ETL开发与数据建模。我开发了100多个离线任务，处理每日5亿条数据记录，并构建了覆盖60多项核心业务报表的商业报告系统。

在美团，我担任核心开发人员，负责两个关键业务领域：用户和订单。我独立完成了核心事实表和维度表的重构，并统一了80多个业务指标的统计口径。此外，我还参与了实时数据仓库的早期建设，并使用Flink开发了多个核心实时监控指标。

在微软工作期间，我负责MSN广告数据处理。我主导了从Scope SQL到Spark Structured Streaming的技术栈迁移，将100多个核心离线及准实时任务从微软自有的Scope平台迁移到开源的Spark生态中。同时，我还进行了全链路性能优化，最终使最终任务的平均运行时间减少了45%，存储消耗降低了35%。此外，我建立了自动化数据质量监控系统，实现了任务的自动部署、测试与告警，并将数据异常检测的粒度从小时级提升至分钟级。
...
我在美团遇到过一次非常严重的数据倾斜问题，是在计算用户订单历史的时候。有几个大商家的订单量特别大，占了总订单量的 20% 以上，导致 Spark 任务中某个 executor 的运行时间是其他 executor 的 10 倍以上，整个任务跑了 6 个多小时还没跑完。
...
我遇到最常见的的数据倾斜是由于上游数据问题导致的空字段或异常值，比如robot导致的。

解决方案：

一劳永逸的方案，是否使用先统计异常值，然后对异常key单独处理的方式。

为了兼顾健壮性和效率，首先采用的是按照重试次数，重试第三次开始使用优化逻辑计算，后面就变成了参数化，因为有时候上游的数据会影响不止一个delta的数据
...
#### 	2.1 你在工作中遇到过哪些类型的数据倾斜？请分别说明它们的解决方法。如果一个大表和一个小表 join 出现数据倾斜，你会怎么解决？如果两个大表 join 出现数据倾斜，你会怎么解决？
...
- 当一方不足，另一方空闲时，均可以借用空闲的一方（是否都没有上限？）
- 当执行内存被占用需要回收时，可以强制驱逐存储内存借用的部分，但不能驱逐分配给存储内存的部分，即存储内存不会被驱逐到低于`spark.memory.storageFraction`的比例
- 当存储内存被占用需要回收时，只能等待执行内存释放，不能驱逐执行内存，在执行内存归还前，Spark 会驱逐存储内存中最近最少使用 (LRU) 的数据块，将其溢写到磁盘
...
### 2. Spark 的核心架构组件有哪些？RDD 的五大特性是什么？
...
1. **Executor 配置**：合理设置 Executor 的数量、每个 Executor 的 Core 数、内存大小。通常每个 Executor 分配 2-5 个 Core，避免 Core 过多导致的线程竞争；内存分为执行内存和存储内存，生产环境通常设置`spark.memory.fraction=0.7`，提升执行内存占比。
2. **并行度设置**：设置合理的任务并行度，通常并行度设置为集群总 Core 数的 2-3 倍，避免并行度过低导致资源闲置，或并行度过高导致任务调度开销大。通过`spark.default.parallelism`（RDD）和`spark.sql.shuffle.partitions`（Spark SQL）设置。
...
2. **调整 Shuffle 参数**：增大 Shuffle 的缓冲区大小`spark.shuffle.file.buffer`，减少溢写磁盘的次数；开启 Shuffle 压缩`spark.shuffle.compress=true`，减少网络传输的数据量。
3. **优化 Shuffle 分区数**：合理设置`spark.sql.shuffle.partitions`，默认 200，数据量大时调大，避免单个分区数据量过大导致的 OOM。
...
4. **使用 Kryo 序列化**：开启 Kryo 序列化`spark.serializer=org.apache.spark.serializer.KryoSerializer`，比 Java 序列化速度更快、占用内存更小。
...
#### 2.1 Checkpoint 实现内部 Exactly-Once 的原理
...
### 3. Flink 是什么？对比 Spark Streaming，为什么 Flink 更适合实时数仓？
...
### 6.【高频考点】Flink 的状态管理是什么？状态后端有哪几种？如何选型？
...
简单来说，反压就是**下游处理不过来，向上游反馈压力，导致整个任务流速下降**。
...
1. 第一步：通过 Flink WebUI 定位反压源头
   ...
   - 状态为 OK：代表该算子无反压，通常是反压的下游瓶颈节点。
     ...
     反压的传播规律是：从下游瓶颈节点，向上游传播，所以 WebUI 中最下游的 OK 状态算子，就是反压的瓶颈源头。
...
1. Producer 端：
   ...
   - 设置`acks=all`（或 - 1）：只有当消息被 Leader 副本和所有 ISR 中的 Follower 副本都成功写入后，才认为消息发送成功，避免 Leader 宕机导致消息丢失。

   - 设置`retries`重试次数：网络波动时，Producer 自动重试发送消息，避免临时故障导致的消息发送失败。
...
3. 注意事项：Producer 端必须关闭重试，或开启幂等性，否则重试可能导致消息乱序。

### 2. Kafka 的核心架构组件有哪些？分别有什么作用？
...
### 1. SQL 中`row_number()`、`rank()`、`dense_rank()`的区别，以及适用场景
...
### 2. Hive 中 order by、sort by、distribute by、cluster by 的区别是什么？
...
4. **cluster by**：相当于`distribute by + sort by`，当 distribute by 和 sort by 的字段完全相同时，可以用 cluster by 简写，既保证相同字段的数据分到同一个 Reduce，又保证 Reduce 内部按该字段有序。缺点是只能升序排序，不能指定排序规则。



### 3. Hive SQL 的通用优化手段有哪些？
...
1. **分区裁剪 + 列裁剪**：查询时必须带分区条件，只查询需要的分区；避免使用`SELECT *`，只查询需要的字段，减少扫描的数据量。
...
5. **控制动态分区**：开启动态分区时，严格控制分区数量，避免生成过多小文件；设置`hive.exec.dynamic.partition.mode=nonstrict`时，必须有一个静态分区。

#### 3.3 参数配置层面优化

1. **开启 CBO 优化器**：`set hive.cbo.enable=true;`，CBO 会自动优化 SQL 的执行计划，选择最优的 Join 顺序、关联方式。
2. **合并小文件**：Map 端合并小文件`set hive.input.format=org.apache.hadoop.hive.ql.io.CombineHiveInputFormat;`；Reduce 端合并小文件`set hive.merge.mapfiles=true;set hive.merge.mapredfiles=true;`，减少 MapTask 数量，降低 NameNode 压力。
3. **调整并行度**：合理设置 MapTask 和 ReduceTask 的数量，MapTask 数量由输入文件的大小和数量决定，ReduceTask 数量通过`set mapreduce.job.reduces=xxx;`设置，避免并行度过低导致任务慢，或并行度过高导致集群资源争抢。
4. **开启数据倾斜优化参数**：如`hive.groupby.skewindata=true`、`hive.optimize.skewjoin=true`，自动处理倾斜场景。



### 4. Hive 数据倾斜的根本原因是什么？有哪些常见场景？对应的解决方案是什么？

**答案**：

- 根本原因：Shuffle 阶段，相同 Key 的数据被分发到同一个 ReduceTask 中处理，出现**部分 Key 对应的数据量远大于其他 Key**，导致少数 ReduceTask 运行时间极长，整个任务等待这几个 ReduceTask 完成，出现任务卡顿、超时。
...
- 常见场景及解决方案：
  ...
  3. 场景 3：大表与大表 Join，Key 分布极度不均

     原因：部分热点 Key（比如大商家、头部用户）对应的数据量极大，集中在少数 Reduce 中。

     解决方案：加盐打散 + 二次聚合。第一步，给倾斜的 Key 加上随机前缀，打散到多个 Reduce 中做局部聚合；第二步，去掉随机前缀，做全局聚合，完成最终计算。

  4. 场景 4：Count (Distinct) 导致倾斜
...
3. 问：脏数据怎么处理
   ...
   答：ODS原样保留；DWD层清洗过滤、空值替换、异常剔除、格式统一；DWS层做业务逻辑校验；建立数据质量监控告警。

4. 问：数据仓库分层的意义

   答：① 解耦：原始数据不改动，中间层隔离；② 复用：明细层通用，上层集市直接复用；③ 容错：分层回溯，出错快速定位；④ 简化开发：每层职责单一；⑤ 口径统一：避免重复计算、重复开发。

5. 问：为什么要做数据仓库

   答：① 业务库承受不了大查询、报表、聚合分析，容易压垮线上业务；② 业务库分散多源，数据孤立，数仓统一整合；③ 业务库无历史快照，无法回溯历史数据；④ 数仓分层建模，指标复用、口径统一；⑤ 数仓专门做OLAP，聚合查询性能更强。
...
从逻辑上说，雪花模型是最合理的，数据冗余少，容易保证数据一致性，但实际生产中是不合适的。雪花模型会导致计算逻辑的复杂度极大提升，

#### 问题：

星型模型会导致数据一致性问题吗，怎么解决？
...
### 3. 数据仓库标准分层架构（ODS/DWD/DWS/DIM/ADS），每层的作用是什么？

数仓分层的核心目的是**解耦、复用、便于问题排查、减少重复计算**，行业通用 5 层架构如下：

1. **ODS 层（Operational Data Store，原始数据层）**：贴源层，直接对接业务源数据，数据结构和源系统保持完全一致，不做深度清洗转换，仅做数据落地，用于数据备份、原始数据回溯，是数仓的数据入口。同步工具：DataX、Canal、SeaTunnel。
...
### 4. 什么是维度建模？星型模型和雪花模型的区别是什么？

- 维度建模是数据仓库主流的建模方式，核心是围绕**业务过程**构建事实表，围绕分析视角构建维度表，通过事实表和维度表的关联，支撑灵活的多维分析，核心是 “事实 + 维度” 的设计思想。
...
- 常见处理方式：
  ...
  2. **SCD2（新增行记录）**：维度变化时，新增一行数据，通过`start_date`（生效时间）、`end_date`（失效时间）、`is_current`（是否当前生效）标记数据的生命周期，保留全量历史数据，是企业生产最常用的方案。
  3. **SCD3（新增字段）**：新增字段保存维度的新旧值，仅保留最近一次的历史，适合维度变化频率极低、仅需对比新旧值的场景。



### 6. Hive 是什么？Hive 和 MySQL 的核心区别是什么？
...
### 7. 完整的数仓建模流程是什么？从需求到落地需要考虑哪些关键点？
...
### 11. PB 级大表 Join、大表聚合的深度优化方案，从模型、存储、计算、架构全链路优化

**答案**：

PB 级大表 Join 和聚合的核心痛点是**数据量极大、Shuffle 开销极高、数据倾斜严重、任务运行时间超长甚至 OOM**，优化必须从全链路入手，而不是只优化 SQL 写法，具体方案如下：
...
存储层优化的核心是**让查询只扫描需要的数据，减少磁盘 IO 和网络传输**，为后续的计算减负。
...
计算层优化的核心是**减少 Shuffle 的数据量，解决数据倾斜，提升并行计算效率**。

1. Join 类型选型优化

   - 大小表 Join：必须使用 Broadcast Join（广播 Join），将小表全量广播到所有 Executor 的内存中，在 Map 端完成 Join，完全避免 Shuffle，这是大小表 Join 的最优解。注意：小表不是绝对的大小，只要能放进内存，就可以使用广播 Join，Spark 中可以通过`spark.sql.autoBroadcastJoinThreshold`调整阈值。
   - 大表大表 Join：优先使用 Bucket Map Join，两个表按相同的 Join 字段分桶，分桶数相同，Join 时只需要对相同分桶的数据做关联，不需要全表 Shuffle，大幅减少 Shuffle 的数据量。
   - 倾斜大表 Join：使用**加盐打散 + 分阶段 Join**，对倾斜的 Key 加上随机前缀，打散到多个 Task 中做局部 Join，再去掉前缀做全局 Join，解决数据倾斜问题。

   

2. 聚合优化，减少 Shuffle 数据量
...
3. Shuffle 优化，降低 Shuffle 开销

   - 调整 Shuffle 分区数：PB 级数据的 Shuffle，默认的 200 个分区完全不够，需要根据数据量调整`spark.sql.shuffle.partitions`，通常按每个分区处理 1GB 数据设置，避免单个分区数据量过大导致的 OOM。
   - 优化 Shuffle 参数：增大 Shuffle 缓冲区`spark.shuffle.file.buffer`，减少溢写磁盘的次数；开启 Shuffle 压缩`spark.shuffle.compress=true`，减少网络传输的数据量；使用高效的 Shuffle 管理器，比如 SortShuffleManager。
...
4. 数据倾斜专项优化
   ...
   - 动态倾斜检测：开启 Spark 的动态倾斜检测参数，比如`spark.sql.adaptive.enabled=true`、`spark.sql.adaptive.skewJoin.enabled=true`，自适应执行引擎会自动检测倾斜的分区，拆分倾斜分区，分散到多个 Task 中处理，这是 Spark 3.0 + 的核心特性，生产环境必须开启。
...
大表join怎么优化？         答：① 大表过滤前置，减少数据量；② 合理分区，按时分区裁剪；③ 小表广播join；④ 倾斜key单独打散；⑤ 开启MapJoin；⑥ 避免笛卡尔积；⑦ 拆分复杂SQL，分步执行。      
...
#### 1. 简述数据仓库的定义、四大核心特性，以及和业务数据库的核心区别

**答案**：

- 定义：数据仓库是**面向主题、集成、相对稳定、随时间变化**的数据集合，核心目标是为企业管理决策、数据分析提供支撑，不参与线上业务交易。
...
- 使用分区表的核心原因：**避免全表扫描，提升查询效率**。查询时可以通过`WHERE`语句过滤分区，只扫描需要的分区数据，不用扫描全表，大幅减少查询的数据量，尤其适合海量数据按时间维度查询的场景。



#### 4.事实表有哪几种类型？分别适用什么场景？
...
Flink 超大状态（TB 级）的核心痛点是**状态持续膨胀、Checkpoint 超时失败、任务重启状态恢复极慢、TaskManager 频繁 OOM**，这类问题通常出现在长窗口计算、海量 Key 的去重、实时用户画像、维表关联等场景，优化方案从状态设计、状态后端、Checkpoint 优化、运维配置四个维度入手，全链路优化：
...
1. **严格控制状态生命周期，开启 TTL 自动清理过期状态**
   ...
   - 注意：Flink SQL 中可以通过`table.exec.state.ttl`参数给 SQL 任务设置全局状态 TTL，生产环境必须开启，禁止无 TTL 的状态。

   

2. **状态瘦身，只存储必要的字段，减少状态冗余**
...
2. **RocksDB 核心参数优化**
   ...
   1. **开启增量 Checkpoint**：`state.backend.incremental=true`，开启后，每次 Checkpoint 只上传和上一次 Checkpoint 相比变化的增量数据，而不是全量状态，TB 级状态的 Checkpoint 时间可以从小时级降到分钟级，彻底解决 Checkpoint 超时问题，生产环境必须开启。
   2. **调整 RocksDB 的内存配置**：默认的 RocksDB 内存配置过小，超大状态场景需要调大，比如`state.backend.rocksdb.memory.managed=true`开启托管内存，`state.backend.rocksdb.memory.write-buffer-ratio=0.5`调大写缓冲区比例，减少磁盘溢写次数，提升写入性能。
   ...
   4. **开启压缩**：`state.backend.rocksdb.compression.type=LZ4_COMPRESSION`，开启 LZ4 压缩，减少状态占用的磁盘空间，同时减少磁盘 IO 开销，对于 TB 级状态，压缩比可以达到 5:1 以上。
...
4. **开启 Changelog 机制，加速 Checkpoint**
...
2. **状态扩容重分布优化**
   ...
   - 任务扩容时，状态会在多个 TaskManager 之间重分布，TB 级状态的重分布耗时极长，优化方案：
     ...
     1. 使用 Key Group 范围优化：Flink 的 Key Group 是状态重分布的最小单元，合理设置`max-parallelism`，避免 Key Group 数量过多或过少，通常设置为并行度的 2 倍，且是 2 的整数次幂。
...
### 7. 实时计算 UV（日活），如何解决数据乱序、重复计算、精准去重的问题？

**答案**：

UV 的核心难点是**用户去重、乱序数据处理、保证精准性**，分低延迟近似方案和精准方案，生产环境根据业务需求选型，具体实现如下：
...
1. **数据乱序问题解决**
   ...
   - 配置 Watermark，设置合理的允许乱序时间，比如`Watermark for event_time as event_time - INTERVAL '30' SECOND`，允许数据最多迟到 30 秒；
   - 开启窗口延迟关闭，设置`allowedLateness(Time.hours(2))`，允许窗口关闭后，2 小时内的迟到数据仍然可以进入窗口参与计算，更新 UV 结果；
...
### 9. 多业务线、多租户场景下，数仓如何设计？如何平衡公共数据复用与业务线数据独立？

**答案**：

多业务线、多租户数仓设计的核心原则是**公共层统一、应用层隔离、资源隔离、权限管控**，既保证全公司数据的一致性、复用性，又满足各业务线的个性化需求，避免数据孤岛，具体设计方案如下：

##### 一、整体架构设计：三层架构体系

整体分为**公共层、业务线中间层、业务线应用层**，实现 “高内聚、低耦合”，从上到下，通用性递减，个性化递增。
...
1. **数据隔离**
   ...
   - 库表隔离：企业级公共层统一在一个库中，各业务线的中间层、应用层，独立建库，库名按业务线命名，比如`dwd_live`、`ads_ecommerce`，避免表名冲突，实现数据隔离；
...
### 6. 设计一套电商场景的离线 + 实时数仓架构，覆盖从数据采集到指标输出的全流程
...
### 8. 【高频考点】设计一套 PB 级海量数据下的离线 + 实时一体化数仓架构，说明技术选型、分层设计、数据流转、核心难点解决方案

**答案**：

##### 一、架构设计总览

整体采用**流批一体 + 湖仓一体**的架构，解决 PB 级海量数据下的存储成本、计算性能、口径一致性、扩展性问题，实现一份数据、一套口径、两种计算模式，技术栈选型如下：
...
### 10. 湖仓一体（LakeHouse）在企业级数仓中的落地方案，对比传统数仓的优势，以及落地过程中的核心坑点与解决方案

**答案**：

##### 一、湖仓一体的核心定义

湖仓一体是一种结合了数据湖和数据仓库优势的新型架构，在低成本的数据湖存储之上，构建数据仓库的 ACID 事务、数据管理、高性能分析能力，实现**一份数据同时支持流处理、批处理、交互式分析、机器学习**，避免传统架构中数据湖和数据仓库之间的数据冗余、搬迁、口径不一致问题。
...
基于**HDFS + Apache Hudi + Flink + Spark**构建企业级湖仓一体架构，落地分为 5 个阶段：
...
1. **坑点 1：小文件爆炸，导致查询性能急剧下降**
   ...
   - 解决方案：
     ...
     2. 开启 Hudi 自动小文件合并：配置`hoodie.parquet.small.file.limit`、`hoodie.compact.inline.max.delta.commits`，开启 inline compaction，自动合并小文件；
     3. 定时离线合并：每日低峰期，通过 Spark 任务执行 Clustering 操作，对历史分区的小文件做全量合并，优化查询性能；
     4. 清理过期提交：设置`hoodie.cleaner.commits.retained`，自动清理过期的 commit 和文件，减少文件数量。

     

   

2. **坑点 2：写入和查询冲突，导致查询超时、写入失败**
...
### 11. 如何设计一套支撑企业数字化转型的企业级数仓架构？从战略、架构、落地、治理全流程说明

**答案**：

企业级数仓建设的核心目标不是单纯的技术落地，而是**支撑企业的数字化转型，用数据驱动业务决策、业务创新、降本增效**，必须从企业战略出发，自上而下设计，而不是单纯的技术堆砌，全流程设计分为 6 个阶段：
...
基于企业战略和业务诉求，设计企业级数据架构，整体采用**流批一体 + 湖仓一体**的云原生架构，实现 “数据一体化、分析一体化、服务一体化”，架构分为 5 层：
...
### 12. 流批一体架构在企业级的落地方法论，对比 Lambda 架构，如何解决落地过程中的业务适配、技术门槛、历史兼容问题？

**答案**：

##### 一、流批一体的核心定义与企业级价值

流批一体是指**用同一套引擎、同一套 SQL、同一套模型，同时处理流数据和批数据**，彻底解决传统 Lambda 架构的两套链路、两套代码、口径不一致、开发维护成本高的核心痛点。
...
1. 核心技术栈选型
   ...
   企业级流批一体的核心是 **计算引擎 + 表格式** 的组合，主流选型方案如下：
...
4. 痛点 4：流批一体的性能问题，尤其是批量历史数据处理性能不如传统批处理
...
5. 痛点 5：数据治理体系不兼容，原有治理工具无法适配流批一体架构
